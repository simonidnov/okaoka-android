...
simplediscs.prototype.get_colors = function(id){
    var level_colors = [];
    var colorer = _.clone(colors);
    for(var i=0; i<4; i++){
        var num = Math.floor(Math.random()*_.keys(colorer).length);
        var color = _.keys(colorer)[num];
        level_colors.push(color);
        delete colorer[color];
    }
    var possibilities = effectue();
    var arrgts = [];
    for(var a=0; a<possibilities.length; a++){
        var num = Math.floor(Math.random()*possibilities.length);
        arrgts[a] = [];
        arrgts[a].push(level_colors[possibilities[num][0]-1]);
        arrgts[a].push(level_colors[possibilities[num][1]-1]);
        arrgts[a].push(level_colors[possibilities[num][2]-1]);
    }
    simplediscs_levels = [];
    for(var i=0; i<arrgts.length; i++){
        delete colorer['black'];
        delete colorer['red'];
        var num = Math.floor(Math.random()*_.keys(colorer).length);
        simplediscs_levels.push(
            {
                bkColor:_.keys(colorer)[num],
                colors:arrgts[i]
            }
        );
    }
...
simplediscs.prototype.create_level = function(id){
    ...
    var other = _.clone(simplediscs_levels);
    other.splice(id, 1);
    for(var i=0; i<4; i++){
        if(i === _okg.response){
            other.splice(id, 1);
            ...
            var ran = Math.floor(Math.random()*other.length);
            _okg.create_choices((i+1), other[ran]);
            other.splice(ran, 1);
            console.log("simplediscs_levels : ", simplediscs_levels.length);
...
var arrgtliste, nbarrgt;

var bij = function(x) {
	return x
}

function arrgt(L, t, n ,k) {
   if(k==0) {
     arrgtliste += ""+L+"|";
     nbarrgt++;
   } else {
...
function effectue() {
    ...
    nbarrgt = 0;
    arrgt(L, t, n, k);
    var probs = arrgtliste.split('|');
    var arrgts = [];
    for(var i=0; i<probs.length; i++){
        if(probs[i] !== ""){
            arrgts.push(probs[i].split(','));
        }
    }
    return arrgts;
}
